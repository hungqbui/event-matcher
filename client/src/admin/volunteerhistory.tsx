...
export default function VolunteerHistory() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/admin/volunteer-attendance?admin_user_id=${user.id}`)
      .then(res => res.json())
  ...
  const handleViewTasks = async (record: VolunteerRecord) => {
    try {
      ...
      // Initialize ratings to 100% for all tasks
  ...
  const handleRateTask = async (taskId: number) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:5000/api/task/${taskId}/rate`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        alert(`Task rated successfully! Score: ${result.actual_score}/${result.original_score} (${result.rating_percent}%)`);
          ...
          <div className="vh-modal-backdrop" onClick={closeModal}>
            <div className="vh-modal" onClick={(e) => e.stopPropagation()}>
              <div className="vh-modal-header">
                ...
                <p className="vh-modal-subtitle">{taskModal.volunteer.eventName}</p>
                <button className="vh-modal-close" onClick={closeModal}>&times;</button>
              </div>
              
              <div className="vh-modal-body">
                {taskModal.tasks.length === 0 ? (
                  <p className="vh-no-tasks">No tasks claimed for this event.</p>
                ) : (
                  <div className="vh-tasks-list">
                    {taskModal.tasks.map((task) => (
                      <div key={task.id} className={`vh-task-card ${task.completed ? 'completed' : ''}`}>
                        <div className="vh-task-header">
                          <h3>{task.name}</h3>
                          <span className="vh-task-score">Max Score: {task.score}</span>
                        </div>
                        
                        {task.completed ? (
                          <div className="vh-task-completed">
                            <span className="vh-completed-badge">✓ Completed & Rated</span>
                            <span className="vh-final-score">Final Score: {task.score}</span>
                          </div>
                        ) : (
                          <div className="vh-task-rating">
                            ...
                            <input
                              ...
                              onChange={(e) => setRatings({...ratings, [task.id]: parseInt(e.target.value)})}
                              className="vh-rating-slider"
                            />
                            <div className="vh-rating-info">
                              <span>Score: {Math.round((task.score * (ratings[task.id] || 100)) / 100)}/{task.score}</span>
                            </div>
                            <button
